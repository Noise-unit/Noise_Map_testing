...
function pick(row, key) {
  ...
  // exact hit first (also BOM-safe)
  const direct = Object.keys(row).find((k) => norm(k) === norm(key));
...
function buildPopupHTML(datasetName, row) {
  const ref = pick(row, "Reference No.") || pick(row, "Reference");
  if (!ref) {
  console.log("⚠️ Reference not found. Available keys:", Object.keys(row));
}
  const year = pick(row, "Year");
  const applicant = pick(row, "Applicant");
  const host = pick(row, "Host");
  const vrType = pick(row, "VR type");
  const determination = pick(row, "Determination");
  const determinationDate = pick(row, "Determination Date");

  const eventLocation = pick(row, "Event Location");
  const eventDesc = pick(row, "Event Description");

  const startDate = pick(row, "Start date");
  const endDate = pick(row, "End date");
  const startTime = pick(row, "Start time");
  const endTime = pick(row, "End time");

  const easting = pick(row, "Easting");
  const northing = pick(row, "Northing");

  const datasetTitle = escapeHTML(datasetName);
  const vrRef = ref ? escapeHTML(ref) : "";

  const badgeText = determination || "—";
  const detNorm = badgeText.toLowerCase();
  const badgeClass =
    detNorm.includes("issued") ? "is-issued" :
...
const eventEndLine = `${
  ...
  const kv = (k, v) => `
    <div class="ema-kv">
      <div class="ema-k">${escapeHTML(k)}</div>
      <div class="ema-v">${escapeHTML(v || "—")}</div>
    </div>
  `;

  const footer =
    (easting && northing)
      ? `<div class="ema-footer">${escapeHTML(`UTM: E ${easting} / N ${northing}`)}</div>`
      : "";

  return `
    <div class="ema-popup-card ema-popup-v2">
      <div class="ema-popup__header">
      ...
      <div class="ema-popup__body">
        ...
        <div class="ema-box">${escapeHTML(eventDesc || "—")}</div>
